 ...
 *
 * Nothing is written twice: `have()` skips anything already on disk, which now
 * also covers the case of a second creature needing a file the first one
 * already fetched — the shared model behind each staged pair, and the handful
 * of small textures several creatures have in common.
 */
async function fetchCreature(creature) {
  ...
  // disk rather than re-downloaded — that is the 15MB a re-run saves.
  ...
  // slots, and they are the whole reason one model can be two creatures.
