...
function resolveInstance(w: World, inst: Instance) {
  ...
  switch (def.rule.type) {
    ...
    case 'lethalGround':
    case 'syncKill':
      // Never resolved. All three are judged continuously in step(): one is a
      // state you are allowed to sit in for a moment, one is a hole in the
      // floor, which has no resolve moment at all, and the third is a health
      // gap between two entities, which is a condition rather than a cast.
      break
...
function allyThink(w: World) {
  ...
  for (const side of [undefined, 'green', 'red'] as (Side | undefined)[]) {
    ...
    // How many are left for you, and WHICH ones.
    //
    ...
    // takes all but the two or three nearest you. Sorted by distance from the
    // player DESCENDING before slicing, because the claim order used to be array
    // order — so the boxes left over were wherever the ring happened to start,
    // and "these ones are yours" was a lottery rather than the ones under your
    // feet.
    //
    // Both halves are gated on the mechanic declaring `soakers`, which is the
    // author opting in to "some of these are yours". Applied to every collect it
    // also re-sorted the Entombed Sentinels' Toxic Droplets — which declare no
    // share and were tuned on the old array order — and the raid swept them so
    // much more efficiently that a CARELESS healer cleared the fight. A tidier
    // raid AI is a real improvement and it is also a balance change to somebody
    // else's fight; the opt-in keeps the two apart.
    //
    // And nothing is reserved for a player the mechanic does not score.
    //
    // `roles` says whose job a mechanic is. Throw Junk's crates are the
    // dps/healer half of a cast whose tank half is watching it happen — the
    // player tanks Iku and never leaves it — so a share reserved for a tank is
    // a share left on the floor. With a `missCost` of a whole raid bar that is
    // a guaranteed wipe at the first window with no action available to the
    // player, which is not a difficult mechanic, it is an unplayable one.
    // The raid claims the lot instead. Boss-agnostic: it reads `roles`, which
    // every mechanic already declares.
    const share = of[0]?.def
    const keep = share === undefined || share.soakers === undefined
      ? undefined
      : (def_scored(w, share) ? share.soakers : 0)
    if (keep === undefined) {
      claimable.push(...of.slice(0, Math.max(0, of.length - 1)))
      continue
    }
    const sorted = [...of].sort((x, y) => dist(y.pos, w.player.pos) - dist(x.pos, w.player.pos))
    claimable.push(...sorted.slice(0, Math.max(0, sorted.length - keep)))
  }
...
function allyMove(w: World, dt: number) {
  ...
  for (const a of w.allies) {
    ...
    //
    // `d` is the EASED step, not the gap. `ease` is dt/lag, which for a raider
    // with the longest reaction time is 0.083 — so a 0.6 test against the eased
    // vector is really a test against a gap of 0.6/0.083 = 7.2 YARDS, and every
    // ally in the raid stops that far short of wherever they were sent. For a
    // station that is invisible; for an errand measured in yards it is the whole
    // errand. A Throw Junk crate is eaten inside 3 yards, so a raider sent to one
    // walked to five, stopped, and watched it until the window closed — and on a
    // fight where a single uncollected crate wipes the raid, that wiped the tank
    // pull at the first window, in every seed, with no action available to the
    // player at all.
    //
    // So an errand that has to be physically ARRIVED AT closes the last few
    // yards. Gated on the pickup declaring `soakers` — the same opt-in the claim
    // logic above uses for "some of these are yours" — because measuring the
    // deadzone honestly for every ally on every fight re-tunes six encounters
    // nobody has re-tuned: it moved twenty cells of the sweep in both
    // directions, including three careless clears. That is a real improvement
    // and it is also somebody else's balance pass; the opt-in keeps them apart.
    const errand = w.fishAlly === a.id || w.instances.some(i =>
      !i.resolved && !i.answered && i.def.rule.type === 'collect'
      // Three yards of slack, because the idle sway in step 6 nudges `want` by
      // up to 2.3 yards after the errand set it — an exact-match test here would
      // never once have fired.
      && i.def.soakers !== undefined && dist(i.pos, a.want) < 3)
    const gap = errand ? Math.hypot(a.want.x - a.pos.x, a.want.y - a.pos.y) : d
    if (gap > 0.6 && d > 0.0001) {
  ...
  for (const inst of w.instances) {
    ...
    switch (def.rule.type) {
      ...
      case 'polarity':
        ...
        // off the gap between two bodies, and an element off the debuff you are
        // still carrying long after the volley resolved. Prompting off the
        // telegraph as well would say the same thing twice and stop saying it at
...
export function step(w: World, input: Input, dtMs: number) {
  ...
  if (w.bossEnergy >= 100 && !activePhase(w)?.endsAtFullEnergy) {
    if (w.boss.atFullEnergy) {
      ...
      killPlayer(w, w.boss.enrageName ?? 'The bar filled — enraged')
      return
    }
  }
